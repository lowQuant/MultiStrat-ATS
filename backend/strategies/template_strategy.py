...
class TemplateStrategy(BaseStrategy):
    ...
    def on_fill(self, trade, fill):
        ...
        add_log(f"Order filled: {fill.execution.shares} shares @ ${fill.execution.price}", self.symbol)
    
    def on_status_change(self, trade):
        """Handle order status changes"""
        super().on_status_change(trade)
        status = trade.orderStatus.status
        add_log(f"Order status: {status} (Order ID: {trade.order.orderId})", self.symbol)
